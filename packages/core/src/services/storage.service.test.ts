import { SESSION_STORAGE_KEYS } from "../message-parser";
...
describe("StorageService", () => {
  ...
  it("new session correctly set", () => {
    ...
    const storedHistory = JSON.parse(
      sessionStorage.getItem(SESSION_STORAGE_KEYS.CHAT_HISTORY) || "{}"
    );
  ...
  it("add a message to an existing session with malformed messages array", () => {
    ...
    const malformedData = JSON.parse(
      sessionStorage.getItem(SESSION_STORAGE_KEYS.CHAT_HISTORY)!
    );
  ...
  it("get chat history from session storage", () => {
    ...
    sessionStorage.setItem(
      SESSION_STORAGE_KEYS.CHAT_HISTORY,
